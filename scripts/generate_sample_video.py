...
COLORS = ['black', 'navy', 'darkgreen', 'maroon', 'purple', 'darkorange', 'darkslategray']


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Generate a deterministic sample video using ffmpeg.')
    parser.add_argument('--output', default='input/fixtures/short_session_video.mp4', help='Output video path.')
...
def build_ffmpeg_command(output: Path, width: int, height: int, fps: int) -> list[str]:
    ...
    for index, (label, duration) in enumerate(SEGMENTS):
        ...
        inputs.extend(['-f', 'lavfi', '-i', f'color=c={color}:s={width}x{height}:d={duration}:r={fps}'])
    ...
    filter_complex = ';'.join(filters) + ';' + ''.join(concat_inputs) + f'concat=n={len(SEGMENTS)}:v=1:a=0[outv]'
    return [
        ffmpeg,
        '-y',
        *inputs,
