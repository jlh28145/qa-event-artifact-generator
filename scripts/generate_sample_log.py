...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate sample log fixtures and event JSON examples."
    )
    parser.add_argument(
        "--output-dir",
        default="input/fixtures",
        help="Directory where fixtures are written.",
    )
    parser.add_argument(
        "--base-time",
        default="2026-04-04T12:00:00",
        help="Base timestamp for log generation.",
    )
...
def main() -> int:
    ...
    Path(output_dir / "happy_path_log.txt").write_text(
        "".join(timestamped(happy)), encoding="utf-8"
    )
    Path(output_dir / "error_path_log.txt").write_text(
        "".join(timestamped(error)), encoding="utf-8"
    )
    Path(output_dir / "mixed_path_log.txt").write_text(
        "".join(timestamped(mixed)), encoding="utf-8"
    )
    ...
    Path(output_dir / "generated_events_example.json").write_text(
